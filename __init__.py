...
class Updater():
    ...
    def update_collection_names(self):
        ...
        collections = [c.name_full for c in active_object.users_collection]        
        collections_text = " | ".join(collections)

        self.collections_text = collections_text

        return True


    # this is a bit weak, lets hope its good enough...
...
def register():
    args = (updater,)    
    updater.draw_handler = bpy.types.SpaceView3D.draw_handler_add(updater.draw, args, 'WINDOW', 'POST_PIXEL')


def unregister():    
    if updater.draw_handler:
        bpy.types.SpaceView3D.draw_handler_remove(updater.draw_handler, 'WINDOW')
